...
Node模块通过本目录`node_modules`连接至既有artifact-tool依赖；该连接不进Git。若仅先运行数值、独立审计、图和报告，可加`--compute-only`，随后运行`node final_run/q2/workbook.mjs <运行目录>`及`python final_run/q2/check_workbook.py <运行目录>`完成Excel。
